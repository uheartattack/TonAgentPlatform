...
### Added — On-chain Agent Passport
- **Studio "On-chain" tab group** — per-agent **Passport** view: the agent's on-chain
  NFT (TON) with original author, current owner, times-sold counter, and a full
  ownership-history timeline; plus a **Network** view listing the cross-owner agent
  registry. Owner-gated "Mint as NFT" action.
- **API** — `GET /api/v3/agents/by-tap/:tapId` (passport by platform agent id) and
  `GET /api/v3/agents` (network registry), public read-only and flag-gated via
  `V3_NETWORK_ENABLED`; owner-gated minting via `POST /api/v3/minter/mint`.
- **Off-chain indexer** — resolves on-chain agent owners to platform users via
  `agentic_wallets` (multi-form address match, BIGINT-safe), and binds sold/minted
  agents to their owner in the registry.
