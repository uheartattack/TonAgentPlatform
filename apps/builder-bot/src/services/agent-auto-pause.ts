...
export async function pauseAgent(
  ...
  try {
    ...
    try {
      // Use the unified notify helper so OIDC-only users (no direct telegram_id in
      // user_id) also receive the alert via their linked Telegram chat.
      const { notifyUserViaTelegram } = await import('./notify-user');
      await notifyUserViaTelegram(userId, text, {
        parseMode: 'HTML',
        buttons: [[{ text: 'Open agent in Studio', url: `https://tonagentplatform.com/studio.html` }]],
      });
    } catch (e: any) {
