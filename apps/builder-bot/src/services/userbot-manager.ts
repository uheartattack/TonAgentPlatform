...
import {
  safeNumber, getSenderId, getChatId, getFromId, getEntityId,
  isGroupMessage, isChannelPost, peerToId,
  buildMediaPrefix, getMediaType, getDocMimeType, getDocFilename,
  getFileSize, getReplyToId, getQuoteText, isBot as isEntityBot,
  getDisplayName, mimeToExtension,
} from './gramjs-utils';
import {
  floodGate, withFloodProtection, ChatProcessingQueue, SendRateLimiter,
  loopGuardCheck, loopGuardCleanup, seemsAutoGenerated, MessageIdDedup,
} from './telegram-flow-control';
...
/** Determine sender rank label for group context (helps AI understand social dynamics) */
function getSenderRank(msg: TgInboxMessage, ownerId?: number): string {
  if (ownerId && msg.senderId === ownerId) return 'owner';
  if (msg.isBot) return 'bot';
  // Could extend with admin detection in the future
  return 'user';
}

/** Context frame — wraps message with metadata for AI context window */
function buildContextFrame(msg: TgInboxMessage, elapsed?: number, ownerId?: number): string {
  ...
  const rank = msg.isGroup ? `[${getSenderRank(msg, ownerId)}] ` : '';
  const safeUserText = `<user_message>\n${msg.text}\n</user_message>`;
  return `[${rank}${name}${elapsedStr} ${time}${media}${reply}] ${safeUserText}${quote}`;
}
...
// ── Anti-loop + dedup from telegram-flow-control.ts (imported above) ──
const _msgDedup = new MessageIdDedup();
...
class UserbotManager {
  ...
  private async _doConnect(agentId: number, sessionString: string): Promise<TelegramClient> {
    ...
    const client = new TelegramClient(session, API_ID, API_HASH, {
      ...
      floodSleepThreshold: 60, // auto-sleep on FLOOD_WAIT up to 60s instead of throwing
    });
  ...
  async startQRLogin(agentId: number, timeoutMs = 120_000): Promise<{ ok: boolean; qrUrl?: string; expiresIn?: number; error?: string }> {
    ...
    const client = new TelegramClient(session, API_ID, API_HASH, {
      connectionRetries: 5, requestRetries: 3, autoReconnect: true, useWSS: false, floodSleepThreshold: 60,
    });
    await client.connect();
  ...
  async enableMessageListener(agentId: number): Promise<boolean> {
    ...
    try {
      ...
      const filter = new NewMessage({ outgoing: false });  // Telethon-style

      const handler = async (event: any) => {
        try {
          const msg = event.message;
          if (!msg || !msg.message) return;
          if (msg.out === true) return; // Double-check outgoing
          const msgFrom = getSenderId(msg) || '?';
          if (msgFrom === selfId) return; // Skip self before logging
          ...
          getChatRing(agentId).add(parsed.chatId, buildContextFrame(parsed, elapsed, cfg?.userId));
  ...
  private async enableAccountListener(tgUserId: number): Promise<boolean> {
    ...
    try {
      ...
      const filter = new NewMessage({ outgoing: false });  // Telethon-style: only incoming
      ...
      const handler = async (event: any) => {
        try {
          ...
          // Double-check outgoing (Telethon always checks msg.out even with outgoing=False filter)
          if (msg.out === true) return;

          const msgFrom = getSenderId(msg) || '?';
          // Skip self-messages before any processing (Telethon's from_users exclude)
  ...
  private async parseMessage(
    ...
    try {
      ...
      // ── Media annotation prefix for AI context (Telethon-style) ──
      const mediaPrefix = buildMediaPrefix(msg);
      ...
      return {
        ...
        isGroup,
        isChannel,
        isBot: isEntityBot(msg.sender),
        mentionsMe,
        replyToId: getReplyToId(msg) ?? undefined,
        quoteText: getQuoteText(msg) || undefined,
        hasMedia: !!msg.media,
        mediaType: getMediaType(msg) !== 'none' ? getMediaType(msg) : undefined,
        _raw: msg,
      };
    } catch (e: any) {
  ...
  private async processTgInboxMessage(
    ...
    // ── Skip likely AI-generated messages from other bots (prevent bot-bot loops) ──
    if (msg.isGroup && msg.senderId !== Number(cfg.userId) && seemsAutoGenerated(msg.text)) {
      console.log(`[UserbotMgr] 🤖 Agent#${agentId} skip AI-like message from=${msg.senderId}: "${msg.text.slice(0, 40)}"`);
      return;
    }
    ...
    try {
      ...
      } else if (msg.hasMedia && (msg.mediaType === 'photo' || msg.text.includes('[photo '))) {
...
РАСПИСАНИЕ: обещал что-то в конкретное время ("в 10 утра", "завтра") → set_next_wake(delay_seconds, reason). Минимум 1800 сек.
...
async function ubGetMessages(client: TelegramClient, chatId: string | number, limit = 20) {
  ...
  return msgs.map((m: any) => {
    ...
    return {
      ...
      fromId: getFromId(m),
      hasMedia: !!m.media,
      mediaType: getMediaType(m) !== 'none' ? getMediaType(m) : null,
    };
  });
}
...
async function ubDownloadMedia(client: TelegramClient, chatId: string | number, messageId: number): Promise<{ buffer: Buffer; filename: string; mimeType: string } | null> {
  ...
  // Determine filename and mime (Telethon-style via gramjs-utils)
  const mediaType = getMediaType(msg);
  let mimeType = getDocMimeType(msg) || (mediaType === 'photo' ? 'image/jpeg' : 'application/octet-stream');
  let filename = getDocFilename(msg) || (mediaType === 'photo' ? `photo_${messageId}.jpg` : `file_${messageId}.${mimeToExtension(mimeType)}`);
...
async function ubGetMediaInfo(client: TelegramClient, chatId: string | number, messageId: number): Promise<any> {
  ...
  const mType = getMediaType(msg);
  return {
    has_media: true,
    type: mType,
    mime_type: getDocMimeType(msg) || (mType === 'photo' ? 'image/jpeg' : 'application/octet-stream'),
    size: getFileSize(msg) || 0,
    filename: getDocFilename(msg) || null,
  };
}
