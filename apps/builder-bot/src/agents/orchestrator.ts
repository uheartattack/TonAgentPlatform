...
export class Orchestrator {
  ...
  private getToolDefinitions(isOwner: boolean): any[] {
    const tools: any[] = [
      ...
      },
      {
        ...
        function: {
          name: 'run_agent',
          description: 'Запустить или остановить существующего агента по его ID.',
          parameters: {
            type: 'object',
            properties: {
      ...
      },
      {
        ...
        function: {
          name: 'edit_agent',
          description: 'Изменить код или настройки существующего агента.',
          parameters: {
            type: 'object',
            properties: {
      ...
      },
      {
        ...
        function: {
          name: 'explain_agent',
          description: 'Объяснить что делает агент — показать описание и логику работы.',
          parameters: {
            type: 'object',
            properties: {
              agent_id: { type: 'number', description: 'ID агента' },
  ...
  private async processWithAITools(
    ...
    try {
      ...
      const systemPrompt = `Ты — умный AI-ассистент платформы TON Agent Platform.
Помогаешь пользователям создавать и управлять AI-агентами для автоматизации задач в TON-экосистеме.

━━━ КОНТЕКСТ ПОЛЬЗОВАТЕЛЯ ━━━
ID: ${userId}${isOwner ? ' (ВЛАДЕЛЕЦ ПЛАТФОРМЫ)' : ''}
Подписка: ${plan.icon} ${plan.name}
Агенты пользователя:
  ${agentsCtx}
Генераций AI использовано: ${genUsed} / ${genLimit}

━━━ ПРАВИЛА ━━━
• Если пользователь описывает задачу для автоматизации → вызови create_agent
• Если просит показать агентов → вызови list_agents
• Если упоминает конкретный #ID агента для действия → вызови нужный инструмент с этим ID
• Если спрашивает цену/аналитику NFT прямо сейчас (не создание) → вызови analyze_nft
• Если просто разговаривает, задаёт вопрос, нужна помощь → отвечай текстом БЕЗ вызова инструментов
    ...
    try {
      const promptGenResp = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: `Ты — генератор AI-агентов для TON Agent Platform.
Пользователь описывает что ему нужно, ты создаёшь system prompt для автономного AI-агента.

Агент будет работать в цикле (каждые N минут) и иметь доступ к этим tools:
- get_ton_balance(address) — баланс TON кошелька
- get_nft_floor(collection) — floor price NFT коллекции
- get_gift_catalog() — каталог Telegram подарков
- get_fragment_listings(gift_name) — листинги на Fragment
- appraise_gift(gift_name) — оценка подарка
- scan_arbitrage() — поиск арбитражных возможностей (подарки)
- get_gift_floor_real(gift_name) — реальная цена через GiftAsset API
- get_gift_sales_history(gift_name) — история продаж
- get_market_overview() — обзор рынка подарков
- get_price_list() — прайс-лист подарков
- scan_real_arbitrage() — арбитраж через реальные API
- get_gift_aggregator(gift_name) — агрегатор цен
- get_user_portfolio(user_id) — портфолио пользователя
- web_search(query) — поиск в интернете
- fetch_url(url) — получение содержимого URL
- get_state(key) — получить сохранённое состояние
- set_state(key, value) — сохранить состояние между тиками
- notify(message) — отправить уведомление пользователю в Telegram
- list_plugins() — доступные плагины
- suggest_plugin(task) — предложить плагин для задачи

ПРАВИЛА:
1. Пиши system prompt на русском если описание на русском, на английском если на английском
2. Prompt должен быть конкретным: что именно делать на каждом тике
3. Включи условия для уведомлений (когда звать пользователя)
4. Используй set_state/get_state для отслеживания изменений между тиками
5. Если нужен мониторинг — опиши алгоритм сравнения с предыдущим состоянием
6. НЕ пиши код — пиши ИНСТРУКЦИЮ для AI, который будет вызывать tools
7. ‼️ НИКАКИХ ВОПРОСОВ К ПОЛЬЗОВАТЕЛЮ — агент должен ДЕЙСТВОВАТЬ сразу. Если конкретные коллекции/адреса/пороги не указаны — выбери разумные дефолты самостоятельно (популярные коллекции: "Plush Pepe", "Heart Locket", "Lol Pop", "Gem"; спред 5%; порог уведомления 10%). Агент никогда не просит уточнений — он действует с умными дефолтами.
8. ‼️ Для подарков: продажа ТОЛЬКО за TON, Tonnel = только покупка (плохая ликвидность для продажи). Апгрейды подарков игнорировать.
9. Системный промпт начинай с "Действуй немедленно на каждом тике:" чтобы агент не задавал вопросов.

Ответь СТРОГО в формате JSON:
{
  "name": "краткое_имя_агента (2-4 слова)",
  "system_prompt": "полный system prompt для агента",
  "summary": "1 предложение что делает агент"
      ...
      systemPrompt = `Действуй немедленно на каждом запуске:

Твоя задача: ${description}

Используй доступные инструменты (tools) для выполнения задачи. Анализируй данные, принимай решения самостоятельно.

Когда находишь важную информацию или возможность — немедленно уведомляй пользователя через notify() с подробным описанием:
- Что нашёл
- Что нужно сделать
- Конкретные цифры и ссылки

Если ничего нового нет — молчи, не засоряй чат. Используй state (get_state/set_state) чтобы не повторять одно и то же.`;
