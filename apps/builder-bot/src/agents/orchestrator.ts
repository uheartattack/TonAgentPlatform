...
• Действуй автономно — не задавай вопросов`;
    ...
    if (/dune|blockchain.*analyt|sql.*chain|onchain.*data|on-chain.*analyt/i.test(systemPrompt)) detectedCaps.push('blockchain_analytics');
    if (/prompt.*template|prompt.*librar|шаблон.*промпт/i.test(systemPrompt)) detectedCaps.push('prompts');
    if (/другой агент|inter.?agent|ask_agent|multi.?agent/i.test(systemPrompt)) detectedCaps.push('inter_agent');
    if (/discord|дискорд|сервер.*бот/i.test(systemPrompt)) detectedCaps.push('discord');
    if (/twitter|твиттер|x\.com|tweet|твит|пост.*x/i.test(systemPrompt)) detectedCaps.push('x_twitter');
    if (/картинк|изображен|image.*gen|генер.*картин|фото|dall-?e|midjourney|flux/i.test(systemPrompt)) detectedCaps.push('media');
    if (/знани[яй]|knowledge|skill.*tree|обуч|учи|wiki|справочник|документац/i.test(systemPrompt)) detectedCaps.push('knowledge');
    if (/безопасн|security|scan.*address|чёрный.*список|blacklist|антифрод|antifraud/i.test(systemPrompt)) detectedCaps.push('security');
    // Security always enabled for financial agents
    if (detectedCaps.includes('wallet') || detectedCaps.includes('defi') || detectedCaps.includes('gifts_market')) { if (!detectedCaps.includes('security')) detectedCaps.push('security'); }
    ...
    for (const cap of detectedCaps) {
      ...
      if (need && !userVars[need.key]) {
        missingKeys.push(need.label);
      }
    }
    if (missingKeys.length > 0) {
      content += `\n⚠️ _Для полной работы нужно:_\n`;
      for (const mk of missingKeys) {
        content += `  • _${esc(mk)}_\n`;
      }
      content += `_Добавьте в 🔑 API ключи \\(Профиль\\)_\n`;
    }
