...
export class Orchestrator {
  ...
  private async resolveNFTCollectionAddress(name: string): Promise<{ address: string; resolvedName: string } | null> {
    ...
    // 4. Tonscan / tonviewer search (last resort)
    try {
      const r = await fetch(
  ...
  private async createAgentFromTemplateCode(
    ...
    try {
      ...
      if (template.id === 'nft-floor-predictor' || template.id === 'nft-floor-monitor') {
        ...
        const collectionName = rawName.replace(/\b(floor|price|нфт|nft|коллекц\w*|collection)\b/gi, '').trim() || 'TON Punks';

        console.log(`[Orchestrator] NFT template: resolving collection "${collectionName}"`);
        const resolved = await this.resolveNFTCollectionAddress(collectionName);
        if (resolved) {
        ...
        finalTriggerConfig = {
          ...
          config: {
            ...
            COLLECTION_NAME: resolved?.resolvedName || collectionName,
