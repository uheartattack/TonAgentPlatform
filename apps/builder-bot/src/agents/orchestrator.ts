...
export class Orchestrator {
  ...
  // Главный метод обработки сообщения — всё идёт через AI роутер
  async processMessage(
    ...
    // ── AI роутер: один вызов определяет что хочет пользователь ──
    const route = await this.routeWithAI(userId, message, isOwner);
  ...
  /** AI роутер — определяет что хочет пользователь и возвращает action.
   *  Заменяет detectIntent() + classifyIntentWithAI() + keyword-паттерны.
   *  Один AI-вызов (≤60 токенов ответ) вместо 100+ строк regex. */
  private async routeWithAI(
    ...
    try {
      // Загружаем агентов пользователя для контекста
      const agentsResult = await this.dbTools.getUserAgents(userId);
      const agents = agentsResult.data || [];
      const agentsCtx = agents.length > 0
        ? agents.map(a => `#${a.id} "${a.name}"${a.isActive ? ' (активен)' : ''}`).join(', ')
        : 'нет агентов';

      const { text } = await callWithFallback([
        {
          role: 'system',
          content: `Ты — роутер запросов платформы TON Agent Platform.
Проанализируй сообщение пользователя и определи что ему нужно.

Агенты пользователя: ${agentsCtx}

Доступные действия (верни ОДНО):
- "create_agent": пользователь хочет СОЗДАТЬ нового агента/автоматизацию/бота/мониторинг.
  Признаки: "создай", "сделай", "хочу агента который...", "мониторь X", "следи за X",
  "проверяй каждые N", "уведомляй когда", "отслеживай", "напиши скрипт", "запускай каждый час",
  "следи за коллекцией X", "мониторинг NFT X" — всё это create_agent.
- "nft_analysis": пользователь хочет ПРЯМО СЕЙЧАС получить аналитику/цену NFT (без создания агента).
  Признаки: "сколько стоит X?", "покажи floor цену", "анализ рынка nft", "как дела у панков",
  "расскажи про коллекцию X", "посмотри цену X сейчас".
- "list_agents": показать список его агентов ("мои агенты", "список", "покажи агентов").
- "run_agent": запустить существующего агента (упоминает #ID или имя из списка выше).
- "edit_agent": изменить существующего агента.
- "delete_agent": удалить агента.
- "explain_agent": объяснить что делает агент.
- "debug_agent": найти ошибки в коде агента.
- "chat": всё остальное — вопросы о платформе, помощь, разговор.

ПРАВИЛО: Если человек описывает задачу для автоматизации (даже если упоминает NFT/TON) — это "create_agent".
Если человек просто спрашивает цену/аналитику прямо сейчас — это "nft_analysis".

Ответь ТОЛЬКО валидным JSON без markdown: {"action": "..."}`
        },
      ...
      if (typeof parsed?.action === 'string') {
        console.log(`[Orchestrator] AI route: "${parsed.action}" for: "${message.slice(0, 60)}"`);
        return parsed;
      }
    } catch (e: any) {
      console.warn('[Orchestrator] routeWithAI failed:', e?.message || e);
    }
    // Fallback — общий чат
    return { action: 'chat' };
  }
    ...
    if (matchedTemplate) {
      ...
      if (hasRequiredPlaceholders) {
        ...
        const extractedName = rawName
          .replace(/(?:floor|price|нфт|nft|коллекц[А-Яа-яёЁA-Za-z0-9_]*|collection)\s*/gi, '')
          .replace(/\s+/g, ' ')
          .trim();
...
${isOwner ? '\nТЫ ОБЩАЕШЬСЯ С ВЛАДЕЛЬЦЕМ ПЛАТФОРМЫ. Можешь давать технические детали, статистику, советы по развитию.' : ''}`;
  ...
  private matchTemplate(description: string): AgentTemplate | null {
    ...
    // Telegram Star Gift / Fragment подарок — ПЕРВЫМ, до NFT матчинга
    // Иначе "мониторь gift Love Potion" попадёт в NFT шаблон
    ...
    // NFT / floor price / getgems / коллекция (GetGems NFT, не Telegram gifts)
    if (/nft|floor\s*price|floor price|коллекц|getgems|punks|nft.*прогноз|предскажи.*цену|прогноз.*nft/.test(d)) {
